...
foreach($xml->channel->item as $item) {
    ...
    if ($row) {
        $post_id = $row->ID;
        echo "Found post ID via SQL: $post_id\n";
        ...
        if ($res === false) {
            echo "SQL UPDATE ERROR: " . $wpdb->last_error . "\n";
        } else {
            echo "SUCCESS: Updated post ID $post_id directly in DB!\n";
        ...
        echo "POST NOT FOUND VIA SQL BY SLUG!\n";
    }
}
